...
## Migrazione futura a Directus
Quando i dati saranno disponibili da endpoint Directus, basterà sostituire in `stores/dataStore.js`:
```js
const records = (tracceData || []).map(normalize)
```
con:
```js
const res = await fetch(`${API_BASE}/items/datarain?...`)
const json = await res.json()
const records = (json.data || []).map(normalize)
```
La cache localStorage TTL 24h funzionerà già così com'è.
