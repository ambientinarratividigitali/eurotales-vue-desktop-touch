...
function buildDrops() {
  ...
  store.tracce.forEach((item, i) => {
    ...
    // Per il title/alt prendo sempre la versione IT come default (è solo
    // accessibilità, non visibile). Il modale poi mostrerà la lingua giusta.
    img.alt = (typeof item.title === 'object' ? item.title.it : item.title) || ''
    img.draggable = false
    ...
    const startDelay = rand(0, 20000) + (i * 800)
    const t = setTimeout(() => {
      activeTimeouts.delete(t)
      dropOnce(img, item)
    }, startDelay)
